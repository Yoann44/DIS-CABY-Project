  /*
    ...
    Adapted by Vaussard Florian (october 2009)
  */
...
inline unsigned char e_crc8_recursive(unsigned char old_crc, unsigned char new_value){
	return  crc_array[(new_value ^ old_crc)&0xff];
}

unsigned char e_crc8_compute(unsigned char* buffer, unsigned char length){
	unsigned char i;
	unsigned char crc = 0xFF;

	for(i = 0; i<length; i++){
		crc = e_crc8_recursive(crc, buffer[i]);
	}
