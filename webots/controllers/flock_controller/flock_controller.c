...
void reynolds_rules() {
	...
	for (j=0;j<2;j++) {
		...
		if (sqrt(pow(avg_loc[0],2)+pow(avg_loc[1],2)) > RULE1_THRESHOLD) {
				cohesion[j] = avg_loc[j] - my_position[j];   // Relative distance to the center of the swarm
			}
		}
	}
	...
	if(USE_SUPERVISOR) {
		...
		if(MIGRATORY_URGE == 0) {
		    ...
		    speed[robot_id][1] -= MIGRATION_WEIGHT * (migr[0] - my_position[0]); //y axis of webots is inverted
		}
	}
}
